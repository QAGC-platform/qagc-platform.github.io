...
## Beta 37 — quieter rows, dialog destination, inbox through the bridge
Collapsed timeline rows show the headline only: step rows drop the inline owner/budget meta, the objective row drops the inline scope/unit/budget line (progress pill kept when tracking); owner — department · directorate (and budget) move to a `title` hover on the row, full details stay in the expanders. The work-mail dialog now carries a destination `<select id="qcmTo">` (all STAFF, default = resolved route) plus the comment box — for both note and agenda. Work mail syncs through the GAS bridge when a key is on the device (`ibPush`/`ibPushUpd`/`ibPull`, throttled 60s, silent without a key): Code.gs gained the `inbox` sheet lane (`inboxAdd`/`inboxList`/`inboxUpdate`, ICOLS/isheet_) — **needs a redeploy of the user's Apps Script**. Optional email nudge: set Script Property `QAGC_INBOX_MAIL` to a watched mailbox (roster has no personal emails, so one office mailbox, off by default).

## Codex design-audit remediation (betas 46–51)
Turn 02 of the exchange produced a page-by-page audit; its findings shipped in five stages. **b46** functional defects: `.fbox.gate` renamed `inkbox` (the AI diagram inherited the sign-in overlay), the risk predicate compared to a value `stOf` never returns, spotlight click delegation, `gxDayAt` direction-awareness, and the undefined tokens `--ink`/`--surface-1`/`--t-md`. **b47** accessibility: `initCards` gives card headers `role=button`/`tabindex`/`aria-expanded`; a delegated Enter/Space handler plus a mutation-observed `kbdPass()` over `KBD_SEL` covers the mouse-only inventory; switches get `role=switch`+`aria-checked`+a derived name; focus-visible ring. **b48** truthful summaries: eyebrows follow `myPerson()`, `drawDir` derives scope from STAFF/ORG and drops fabricated queue rows, plan tiles derive counts with a zero guard, CPD computes real durations, `todayS=PLATFORM_TODAY`, mine empty states. **b49** localisation: `GX_DIMS_B`/`GX_REPORTS_B`/`GX_TYPES_EN`/`GX_WD_EN` with a `lab:` mapper on `gxF`'s select builder (Arabic keys stored, labels localised); CPD chips pick language before truncating; `.tnum` isolates LTR. **b50** `planView()` gates refs/tags/percentages/budget pills to non-staff roles. **b51** contrast: ink-3/ink-4/gold-ink/rail-muted raised to WCAG AA.
